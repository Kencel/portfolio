...
export function MenuView({ hovered, muted, onToggleMute, onEnter, onOpen, onClearHover }: {
  ...
  return (
    ...
    <div style={{ position: 'relative', zIndex: 5, width: '100%', height: '100vh', overflowY: 'auto', padding: 'clamp(20px,4vw,64px)' }}>

      {/* top-left title + menu column — PROTOTYPE line 52 */}
      <div style={{ position: 'absolute', top: 'clamp(18px,3vw,46px)', left: 'clamp(24px,4vw,60px)', bottom: 'clamp(20px,3vw,44px)', zIndex: 6, display: 'flex', flexDirection: 'column', gap: 'clamp(14px,2.2vh,28px)', width: 'min(66vw,780px)' }}>
        <div>
        ...
        <div onMouseLeave={onClearHover} style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(6px,1vh,11px)' }}>
          {SECTIONS.map((s, i) => (
            <MenuRow key={s.id} section={s} index={i} hovered={hovered} onEnter={onEnter} onOpen={onOpen} />
          ))}
        </div>

        {/* status bars — PROTOTYPE lines 75-84 */}
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 10, width: 'min(360px,100%)', background: '#0b0a0a', border: '2px solid #2a2727', padding: '14px 18px', boxShadow: '4px 4px 0 rgba(228,0,43,.45)' }}>
          <div>
      ...
      <div style={{ position: 'absolute', right: 'clamp(20px,4vw,70px)', top: '50%', transform: 'translateY(-52%) skewX(-6deg)', zIndex: 6, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
